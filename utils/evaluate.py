...
def evaluate_padim_split(padim, loader):
    ...
    for batch in tqdm(loader, desc="Evaluating PaDiM", unit="batch"):
        imgs = batch['image']; structs = batch['structural']; labs = batch['label'].numpy()
    ...
    scores = np.array(y_scores); y_true = np.array(y_true)
    return scores, y_true

# -- main evaluate --
def main():
    parser = argparse.ArgumentParser(description="Unified evaluation for SVDD and PaDiM")
    ...
    if args.mode == 'svdd':
        ...
        for name, root in tqdm([('Balanced', args.balanced), ('Unbalanced', args.unbalanced)]):
        ...
        for name, loader in tqdm([('Balanced', bal_loader), ('Unbalanced', unbal_loader)]):
