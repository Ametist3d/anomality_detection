...
def infer_padim(args):
    ...
    if args.sort_out:
        ...
        print(
            f"Saved score histogram to {os.path.join(args.sort_out, 'score_distribution.png')}"
        )

    # Single-image mode
    else:
        label, score, heatmap = classify_padim(
            padim, config.DEVICE, args.image, args.threshold
        )
        print(f"Image         : {args.image}")
...
def main():
    ...
    parser.add_argument(
        "--threshold", type=float, help="Override the anomaly threshold"
    )

    # SVDD-only options
    parser.add_argument(
        "--ckpt", type=str, default=config.SVDD_MODEL, help="Path to SVDD checkpoint"
    )
    parser.add_argument(
        "--move",
        action="store_true",
